...
def create_second_onto():
    ...
    classes = [["beverage", None], ["water", "beverage"]]
...
def modify_onto():
    ...
    ins = [
        ["Her_pizza", "quattro_stagioni", None, None, None],
    ...
    ontor3.add_distinctions(
        [["classes", ["human", "pizza"]], ["classes", ["has_base", "has_topping"]]]
    )
    ...
    _test_rm(
        ontor3.get_class_restrictions("pizza_company"),
        ["onto-ex.likes.some(onto-ex.food)"],
    ...
    _test_rm(
        ontor3.get_class_restrictions("margherita_company", res_only=False),
        ["onto-ex.company"],
        "pizza_company",
    )
    ...
    ontor3.visualize(
        classes=["human", "pizza"],
        properties=["likes", "diameter_in_cm"],
        focusnode="John",
        radius=2,
        bylabel=False,
        lang=None,
...
def _test_rm(as_is: list, as_expected: list, elem: str) -> None:
    ...
    if set(as_is) == set(as_expected):
        print(
            f"successfully removed {elem} (reparented subclasses and instances if applicable)"
        )
    else:
